package trinsdar.ic2c_extras.util;

import ic2.core.inventory.container.ContainerComponent;
import ic2.core.inventory.gui.custom.MachineGui;

public class GuiMachine {
    public static class RollerGui extends MachineGui {
        public RollerGui(ContainerComponent container) {
            super(container);
        }
    }

    public static class ExtruderGui extends MachineGui {
        public ExtruderGui(ContainerComponent container) {
            super(container);
        }
    }

    public static class CutterGui extends MachineGui {
        public CutterGui(ContainerComponent container) {
            super(container);
        }
    }

    public static class OreWashingPlantGui extends MachineGui {
        public OreWashingPlantGui(ContainerComponent container) {
            super(container);
        }
    }

    public static class ThermalCentrifugeGui extends MachineGui {
        public ThermalCentrifugeGui(ContainerComponent container) {
            super(container);
        }
    }

    public static class MetalPressGui extends MachineGui {
        public MetalPressGui(ContainerComponent container) {
            super(container);
        }
    }
}
