...
public class ContainerNuclearJetpackNew extends ContainerItemComponent<NuclearJetpackInventory> {
    public ContainerNuclearJetpackNew(NuclearJetpackInventory inv, int id, InventoryPlayer player) {
        ...
        ItemStack armor = inv.getInventoryStack();
        if (armor.getItem() instanceof INuclearJetpackLogic){
            INuclearJetpackLogic item = (INuclearJetpackLogic) armor.getItem();
            this.addComponent(new GuiComponentNuclearReactorEuPerTick(item.getLogic(player.player, armor), new Box2D(108, 129, 97, 13)));
            this.addComponent(new GuiComponentNuclearReactorHeat(item.getLogic(player.player, armor), new Box2D(7, 129, 100, 13)));
        }
    }
